create table if not exists public.authenticated_api_rate_limits (
...
alter table public.authenticated_api_rate_limits enable row level security;

revoke all on table public.authenticated_api_rate_limits from anon;
revoke all on table public.authenticated_api_rate_limits from authenticated;
...
revoke all on function public.check_authenticated_api_rate_limit(text, integer, integer) from public;
